...
function ProblemPage() {
    ...
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          ...
          const response = await axios.post(`http://localhost:${process.env.PORT}/api/transcribe`, formData, {
            headers: { "Content-Type": "multipart/form-data" },
            responseType: 'blob',
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        const response = await axios.get(`http://localhost:${process.env.PORT}/api/problem/${id}`);
  ...
  const handleStartInterview = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:${process.env.PORT}/api/start`, {
        title: problem.title,
